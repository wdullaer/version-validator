...
Generates a middleware which will validate according to the options
...
You can pass an array of supported versions in directly, as a shorthand for: `{versions: ['1.0.0', ...]}`

### isVersion(version)
Generates a middleware which calls `next()` if `req.matchedVersion` matches `version` and `next('route')` if it doesn't
